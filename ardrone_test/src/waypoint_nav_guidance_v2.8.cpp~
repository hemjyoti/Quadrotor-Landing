...
void tau(double x_d, double y_d, double z0)
{
	...
	do{
		...
		if(z>0.1 || i<2)
		{
			...
			lx = - Kd*0.02*(drone_navdata.vx*0.001-dxg) - Kp*0.6*(x-xg);    ///changed kd from 0.3
			ly = - Kd*0.02*(drone_navdata.vy*0.001-dyg) - Kp*0.6*(y-yg);
			lz = -((0.7)*(z-zg)); //
...
*/
	...
	//	y = K_est_y;
		...
		loop_rate.sleep();
			...
			}while((r >= 0.2) && (ros::ok()) && (redX==0 || z>0.55));
	//	myfile.close();
		ROS_INFO("Tau over!!!!!");
}
...
void moveto1(double x, double y, double z)
{
	...
	do{
		...
		if(r > 0.15)
		{
			Vx = (drone_navdata.vx*0.001);
			Vy = (drone_navdata.vy*0.001);
			lx =  - Kp*0.25*(e_x)- Kd*0.03*Vx; //moves fast if velocity is given high
			ly = - Kp*0.25*(e_y)- Kd*0.03*Vy;
			lz = 0;
...
*/
		...
		if(z>0.1 || i<2)
		{
			//lx = 1/k1*(k2*drone_navdata.vx*0.001 + axg - Kd*0.3*(drone_navdata.vx*0.001-dxg) - Kp*(x-xg));
			//ly = 1/k3*(k4*drone_navdata.vy*0.001 + ayg - Kd*0.3*(drone_navdata.vy*0.001-dyg) - Kp*(y-yg));
			lx = - Kd*0.1*(drone_navdata.vx*0.001-dxg) - Kp*0.8*(x-xg);
