...
int to, la, f = 190 , redX, redY, res =0, H=0,I=0,J=0;
...
void moveto1(double x_d, double y_d, double z_d);
...
int main(int argc, char **argv)
{
	...
	gps_subscriber = n.subscribe("/ardrone/navdata_gps", 200, gpsCallback); //initialize to receive gps data
...
while(ros::ok())
{
	...
	*/
	...
	case 'q':
			//cout<<endl<<"Height"<<endl;                               // comment height part while using GPS
			//cin>>z;
			//moveup(z+0.5);//tau(x,y,z);

			while(kinect_ready == 0)									//gps recording values
				record();
...
void tau(double x_d, double y_d, double z0)
{
	...
	do{
		...
		if(z>0.2 || i<2)
		{
			...
			lx = - Kd*0.3*(drone_navdata.vx*0.001-dxg) - Kp*0.8*(x-xg);    ///changed kd from 0.3
		...
		myfile<<K_est_x<<setw(25);
		myfile<<K_est_y<<setw(25);
		myfile<<K_est_z<<setw(25);

		myfile<<test_x<<setw(25);
		myfile<<test_y<<setw(25);
		myfile<<test_z<<setw(25);

		myfile<<drone_navdata.altd<<setw(25);

		myfile<<drone_navdata.tm<<setw(25);
	...
	//	myfile<<drone_navdata.header.stamp<<setw(25);
		...
		}while((r >= 0.15) && (ros::ok()) && ((redX==0)||z>0.2));
	//	myfile.close();
		ROS_INFO("Tau over!!!!!");
		//while(I<2000)
		//	I++;
...
*/
		...
		if(z>0.1 || i<2)
		{
			//lx = 1/k1*(k2*drone_navdata.vx*0.001 + axg - Kd*0.3*(drone_navdata.vx*0.001-dxg) - Kp*(x-xg));
			//ly = 1/k3*(k4*drone_navdata.vy*0.001 + ayg - Kd*0.3*(drone_navdata.vy*0.001-dyg) - Kp*(y-yg));
			lx = - Kd*0.005*(drone_navdata.vx*0.001-dxg) - Kp*0.7*(x-xg);
			ly = - Kd*0.005*(drone_navdata.vy*0.001-dyg) - Kp*0.7*(y-yg);
			lz = -((0.7)*(z-zg));
			//cout<<"Cmd_vel"<<lz<<endl;
		}
		else
