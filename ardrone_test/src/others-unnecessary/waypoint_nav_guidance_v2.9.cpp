...
void k_hold()
{
	...
	do
	{
		...
		if(kinect_ready>5 && H<1)
		{
			hover();
			cout<<"tautautautau"<<endl;
			tau(-K_est_x-0.1,-K_est_y,z);
			//land();
			//hover();
			hover();
			while(H<100)
...
void tau(double x_d, double y_d, double z0)
{
	...
	do{
		...
		if(z>0.1 || i<2)
		{
			...
			lx = - Kd*0.01*(drone_navdata.vx*0.001-dxg) - Kp*0.7*(x-xg);    ///changed kd from 0.3
			ly = - Kd*0.01*(drone_navdata.vy*0.001-dyg) - Kp*0.7*(y-yg);
			lz = -((0.7)*(z-zg)); //
...
*/
		...
		loop_rate.sleep();
			...
			}while((r >= 0.2) && (ros::ok()) && (redX==0 || z>0.25));
	//	myfile.close();
		ROS_INFO("Tau over!!!!!");
		//while(I<2000)
		//	I++;
...
*/
		...
		if(z>0.1 || i<2)
		{
			//lx = 1/k1*(k2*drone_navdata.vx*0.001 + axg - Kd*0.3*(drone_navdata.vx*0.001-dxg) - Kp*(x-xg));
			//ly = 1/k3*(k4*drone_navdata.vy*0.001 + ayg - Kd*0.3*(drone_navdata.vy*0.001-dyg) - Kp*(y-yg));
			lx = - Kd*0.05*(drone_navdata.vx*0.001-dxg) - Kp*0.8*(x-xg);
			ly = - Kd*0.05*(drone_navdata.vy*0.001-dyg) - Kp*0.8*(y-yg);
			lz = -((0.7)*(z-zg));
			//cout<<"Cmd_vel"<<lz<<endl;
		}
		else
