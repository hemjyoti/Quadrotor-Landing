...
#include "ardrone_autonomy/RecordEnable.h"	// For Recording video
...
ros::ServiceClient client1;		// ardrone camera record service

using namespace std;

float lx, ly, lz, ax, ay, az,foc = 685, angx,angy,angz, est_x, est_y, est_z,K_est_x, K_est_y, K_est_z;
int to, la, f = 10, redX, redY, res =0, H=0;
...
double Kp = 0.25, Kd = 1,Kpz;							// Gain for the controller
...
// basic func
void hover();		// hovering
void hover1();		// hovering and pushing back with velocity feedback
void hover2();		// hovering and pushing back with acceleration feedback
void hover3();		// hovering and pushing back with corrected acceleration feedback
void takeoff();		// take off
void land();		//landing
void move(float lx, float ly, float lz, float ax, float ay, float az ); // publishing command velocity
void moveacc(float x, float y); // publishing command velocity with acceleration feedback
void moveaccn(float x, float y);//publishing command velocity with normal acceleration feedback
...
int main(int argc, char **argv)
{
	...
	srv.request.channel = 1;	//1 for bottom camera
	if (client.call(srv))
	{
		ROS_INFO("Called");
	}
	...
	//calling camera record service

	client1=n.serviceClient<ardrone_autonomy::RecordEnable>("/ardrone/setrecord",1);

	ardrone_autonomy::RecordEnable srv1;
	srv1.request.enable = 1;	// 1 to enable
...
void k_hold()
{
	...
	ofstream myfile;
	myfile.open("/home/icgel/landing.txt", ios::out | ios::app );
...
void INS_K_Callback(const ardrone_test::est_co::ConstPtr& esti)
{
  //cout<<"Target:"<< val->x<< val->y;
	...
	K_est_y = esti->y;
	K_est_z = esti->z;
	ROS_INFO("hi = %f, %f, %f",K_est_x,K_est_y,K_est_z);
	//cout<<"Target:"<< est_x<< setw(25)<<est_y<<setw(25) << est_z;
}
...
void hover()
{
	...
	dt = t1-t0;
				...
				myfile<<K_est_x<<setw(20);
				myfile<<K_est_y<<setw(20);
				myfile<<K_est_z<<setw(20);
*/
				...
				myfile<<drone_navdata.rotX<<setw(20);
				myfile<<drone_navdata.rotY<<setw(20);
				myfile<<drone_navdata.rotZ<<setw(20);
...
void hover1()//velocity feedback and moving back
{

	double t0 = ros::Time::now().toSec(); //ros has a 'Time' function in which the current time can be found by using 'now'.                                         							we need time in sec to compute hence 'tosec'
	double t1,dt,i=0,r=0,sx=0,sy=0,Vx,Vy,xg=0,yg=0,dxg=0,dyg=0,x,y;

	ros::Rate loop_rate(10);
	ofstream myfile;
			myfile.open("/home/icgel/hover1.txt", ios::out | ios::app );
	do{
	...
	if(r>1)
	{
		...
		lx = - Kd*0.3*(drone_navdata.vx*0.001) - Kp*0.8*(x-xg);
		ly = - Kd*0.3*(drone_navdata.vy*0.001) - Kp*0.8*(y-yg);
		...
		r=getDistance(0,0,0,(x-sx),(y-sy),0);
...
void hover2()//acceleration feedback and moving back
{

	double t0 = ros::Time::now().toSec(); //ros has a 'Time' function in which the current time can be found by using 'now'.                                         							we need time in sec to compute hence 'tosec'
	double t1,dt,i=0,r=0,sx=0,sy=0,Vx,Vy,xg=0,yg=0,dxg=0,dyg=0,Ax,Ay,Az,Ax1,Ay1,Az1,Ax0=0,Ay0=0,x,y;


	ros::Rate loop_rate(10);
	ofstream myfile;
			myfile.open("/home/icgel/hover2.txt", ios::out | ios::app );
	do{
	...
	if(r>1)
	{
		...
		do{
			t1 = ros::Time::now().toSec(); //ros has a 'Time' function in which the current time can be found by using 'now'.                                         							we need time in sec to compute hence 'tosec'
				...
				Ax=drone_navdata.ax*9.78007;
			Ay=drone_navdata.ay*9.78007;
			Az=drone_navdata.az*9.78007;

			Vx=Ax*dt;
			Vy=Ay*dt;

		lx = - Kd*0.05*Vx - Kp*0.25*(x-xg);
		ly = - Kd*0.05*Vy - Kp*0.25*(y-yg);
	...
	else
	{
		...
		sx=sx+(Ax+Ax0)/2*dt*dt;
		sy=sy+(Ay+Ay0)/2*dt*dt;
...
void hover3()//corrected acceleration feedback and moving back
{

	double t0 = ros::Time::now().toSec(); //ros has a 'Time' function in which the current time can be found by using 'now'.                                         							we need time in sec to compute hence 'tosec'
	double t1,dt,i=0,r=0,sx=0,sy=0,Vx,Vy,xg=0,yg=0,dxg=0,dyg=0,Ax,Ay,Az,Ax1,Ay1,Az1,Ax0=0,Ay0=0,x,y;
	double f1[]={0.9941,0.0037,0.0587,0.0028,0.9972,-0.0376,0.0692,-0.0433,-0.9848};
	double f2[]={0.000069405,0.0000018051,-0.00014501};
	double b[]={0.2673,-0.5241,0.7601};

	ros::Rate loop_rate(10);
	ofstream myfile;
			myfile.open("/home/icgel/hover.txt", ios::out | ios::app );
	do{
	...
	if(r>1)
	{
		...
		do{
			t1 = ros::Time::now().toSec(); //ros has a 'Time' function in which the current time can be found by using 'now'.                                         							we need time in sec to compute hence 'tosec'
				...
				Ax1=drone_navdata.ax*9.78007;
			Ay1=drone_navdata.ay*9.78007;
			Az1=drone_navdata.az*9.78007;


						//Removing static bias and scale-factor
						Ax = f1[1]*Ax1+f1[2]*Ay1+f1[3]*Az1-b[1];
						Ay = f1[4]*Ax1+f1[5]*Ay1+f1[6]*Az1-b[1];
						Az = f1[7]*Ax1+f1[8]*Ay1+f1[9]*Az1-b[1];

						Ax=(-1+sqrt(1+4*f2[1]*Ax))/(2*f2[1]);
						Ay=(-1+sqrt(1+4*f2[2]*Ay))/(2*f2[2]);
						Az=-(-1+sqrt(1+4*f2[3]*Az))/(2*f2[3]);
...
void moveacc(float x, float y)//corrected acceleration feedback
{
	...
	double e_x, e_y, e_z,r,r0, i=0,z0, Vx, Vy, Vx0=0, Vy0=0,x_d =0,y_d =0,z_d =0, Ax, Ay, dt, Ax0=0, Ay0=0, Az, Ax1, Ay1, Az1;
	z0 = drone_navdata.altd*0.001;
	double f1[]={0.9941,0.0037,0.0587,0.0028,0.9972,-0.0376,0.0692,-0.0433,-0.9848};
	double f2[]={0.000069405,0.0000018051,-0.00014501};
	double b[]={0.2673,-0.5241,0.7601};
	...
	r0 = getDistance(0, 0, 0, x, y, 0);

	ros::Rate loop_rate(f);

	ofstream myfile;
	myfile.open("/home/icgel/mov_accf.txt", ios::out | ios::app );
...
void moveaccn(float x, float y)//normal acceleration feedback
{
