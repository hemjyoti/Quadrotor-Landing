...
int to, la, f = 5 , redX, redY, res =0, H=0,I=0,J=0;

double k = 0.51, z, lat, lon, ele, tf, ang_time; // k value for tau
...
void record();
void testdataCallback(const ardrone_test::est_co::ConstPtr& test);
...
int main(int argc, char **argv)
{
	...
	test_sub = n.subscribe("/test", 200, testdataCallback);
...
void tau(double x_d, double y_d, double z0)
{
	...
	do{
		...
		if(z>0.1 || i<2)
		{
			...
			lx = - Kd*0.001*(drone_navdata.vx*0.001-dxg) - Kp*0.6*(x-xg);    ///changed kd from 0.3
			ly = - Kd*0.001*(drone_navdata.vy*0.001-dyg) - Kp*0.6*(y-yg);
			lz = -((0.7)*(z-zg)); //
...
*/
		...
		}while((r >= 0.2) && (ros::ok()) && ((redX==0)||z>0.4));
	//	myfile.close();
		ROS_INFO("Tau over!!!!!");
		//while(I<2000)
		//	I++;
