...
#define order 30                                             ////change order for median filter
...
float tmp_depth=0,sampled_depth=0;
int sampled_px=0,sampled_py=0;


float depth_array[order]={0},fltr_depth_val=0;
...
// 	 	}
 ...
 //	prev_depth = depth_val;
 	...
 	//	ROS_INFO("%d",flag1);
 		...
 		if(flag1==31)
 		{
 			med_filter(order);    /////put odd values only
...
*/
 ...
 //	ROS_INFO("%f",fltr_depth_val);

 	if((px!=0 || py!=0 ) && tmp_depth!=0)
 	{

 		sampled_px = sampled_px + px;
 		sampled_py = sampled_py + py;
 		sampled_depth = sampled_depth + tmp_depth;
 		sample--;
 	}

 	if(sample==0)
 	{
 		sampled_depth = sampled_depth/6;
 		sampled_px = sampled_px/6;
 		sampled_py = sampled_py/6;
		...
		val.x = sampled_px;
 		...
 		x_m = sampled_depth-0.1;       //in metres
 		y_m = tan((val.x-512/2)*84.1*deg2rad/512)*(x_m);
 		z_m = tan((val.y-424/2)*53.8*deg2rad/424)*(x_m);
 		...
 		z_m = (float) (z_m/100);
 			...
 			value.x = x_m;
 	 		...
 	 		sample=6;sampled_px=0;sampled_py=0;sampled_depth=0;
