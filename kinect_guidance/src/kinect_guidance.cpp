...
ardrone_autonomy::vector31 test_val;
...
#define order 5                                            ////change order for median filter
...
int flag1=0,i=0,wait=0,flag3=0;
...
void callback( const sensor_msgs::ImageConstPtr& msg_rgb , const sensor_msgs::ImageConstPtr& msg_depth )
{
  	...
  	catch (cv_bridge::Exception& e)
    {
 	...
 	// ROS_INFO("depth_count = %d",depth_count);
 	if((px==0) && (py==0))
 	 {
 	...
 	// 	flag2=0;
 	...
 	else if(depth_val>0 && (abs(prev_depth_val- depth_val)<1000))
 	 {
 		...
 		test_val.x = x_m; test_val.y = y_m ; test_val.z = z_m;
 		test_pub.publish(test_val);
 		ROS_INFO("test co-ordinates = %f, %f, %f",test_val.x,test_val.y,test_val.z);
 		...
 		if(flag1==6)
 		{
 			...
 			med_filter_y(order);
 			med_filter_z(order);
 			prev_depth_val = fltr_depth_val*1000;
 			flag3=1;
 		}
 	...
 	if(sample==0)
 	{
 		...
 		sampled_x_m = (int) (sampled_x_m*1000);
 		sampled_x_m = (float) (sampled_x_m/1000);
 		sampled_y_m = (int) (sampled_y_m*1000);
 		sampled_y_m = (float) (sampled_y_m/1000);
 		sampled_z_m = (int) (sampled_z_m*1000);
 		sampled_z_m = (float) (sampled_z_m/1000);
...
int main(int argc, char **argv)
{
    ...
    */
     ...
     test_pub = nh.advertise<ardrone_autonomy::vector31>("test", 1000);
